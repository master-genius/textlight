...
function fmtDoc(d) {
  let rr = d.id.split('/');
  let backimg = `background-image:url('/m/localimage/${d.headimg}');background-repeat:no-repeat;background-size: 50%;background-position:right top;`;
  return `<div class="card" style="height:7.2rem;${d.headimg.length > 0 ? backimg : ''}" title="${d.name}">
  ${d.headimg.length > 0 ? '<div style="background:rgba(235,236,239, 0.15);text-shadow: 0.2rem 0.22rem 0.15rem #dfdfdf; height:7.2rem;">' : ''}
    <a href="/show?id=${d.id}" target="_blank">
    <h5 style="color:#4a4a4f;" class="title-inline">${d.name.trim()}</h5>
    <p style="color:#767879;font-size:82%;">@${rr[0]} ${rr.length > 1 ? rr[1] : ''}</p>
    <p style="color:#767879;font-size:82%;">
     ${d.time}
    </p>
...
function searchClickTag (tag, closeDrawer = '') {
  ...
  if (closeDrawer === 'close') {
    ...
    document.getElementById('close-drawer').dispatchEvent(evt);
  }
}
  ...
  return apiCall('/mapi/group').then(d => {
    if (d.status === 'OK') {
      ...
      for (let i=0; i< tags.length; i++) {
        ...
        drawerht += `<p><a href="javascript:searchClickTag('${tags[i]}', 'close');" style="color:#bc5920;padding:0.68rem;">· ${tmpname}</a></p><br>`;
      }
      td.innerHTML = ht;

      try {
        document.getElementById('menu-drawer').innerHTML += drawerht;
      } catch (err){}
    }
  })
